/**
 *  \file src/CSR/MatTrisolve/lower_conj.c
 *  \brief Conjugate sparse triangular solve implementation when
 ...
 */
static void
CSR_MatConjTrisolveLower_Unitdiag_xsX(
	...
	oski_value_t alpha, oski_value_t* restrict x, oski_index_t incx )
{
	...
	for( i = 0; i < m; i++ )
	{
		...
		for( k = ptr[i]-indbase; k < ptr[i+1]-indbase; k++ )
		{
			...
			VAL_MSUB_CONJ( _xi, val[k], x[j*incx] );
		}
 ...
 */
static void
CSR_MatConjTrisolveLower_Unitdiag_xs1(
	oski_index_t m, oski_index_t indbase,
	const oski_index_t* restrict ptr, const oski_index_t* restrict ind,
	const oski_value_t* restrict val,
 ...
 */
static void
CSR_MatConjTrisolveLower_Diag_Sorted_xsX( oski_index_t m, oski_index_t indbase,
	const oski_index_t* restrict ptr, const oski_index_t* restrict ind,
	const oski_value_t* restrict val,
	oski_value_t alpha, oski_value_t* restrict x, oski_index_t incx )
{
 ...
 */
static void
CSR_MatConjTrisolveLower_Diag_Sorted_xs1( oski_index_t m, oski_index_t indbase,
	const oski_index_t* restrict ptr, const oski_index_t* restrict ind,
	const oski_value_t* restrict val,
	oski_value_t alpha, oski_value_t* x )
{
 ...
 */
static void
CSR_MatConjTrisolveLower_Diag_Unsorted_xsX( oski_index_t m, oski_index_t indbase,
	const oski_index_t* restrict ptr, const oski_index_t* restrict ind,
	const oski_value_t* restrict val,
	oski_value_t alpha, oski_value_t* restrict x, oski_index_t incx )
{
 ...
 */
static void
CSR_MatConjTrisolveLower_Diag_Unsorted_xs1( oski_index_t m, oski_index_t indbase,
	const oski_index_t* restrict ptr, const oski_index_t* restrict ind,
	const oski_value_t* restrict val,
	oski_value_t alpha, oski_value_t* x )
{
	...
	oski_index_t rowinc, oski_index_t colinc )
{
	...
	if( is_unit )
	{
		if( rowinc == 1 )
			for( k = 0, xp = x; k < num_vecs; k++, xp += colinc )
				CSR_MatConjTrisolveLower_Unitdiag_xs1( m, indbase,
					ptr, ind, val, alpha, xp );
		else /* rowinc > 1 */
			for( k = 0, xp = x; k < num_vecs; k++, xp += colinc )
				CSR_MatConjTrisolveLower_Unitdiag_xsX( m, indbase,
	...
	else /* ! is_unit */
	{
		if( is_sorted )
		{
			if( rowinc == 1 )
				for( k = 0, xp = x; k < num_vecs; k++, xp += colinc )
					CSR_MatConjTrisolveLower_Diag_Sorted_xs1( m, indbase,
						ptr, ind, val, alpha, xp );
			else /* rowinc != 1 */
				for( k = 0, xp = x; k < num_vecs; k++, xp += colinc )
					CSR_MatConjTrisolveLower_Diag_Sorted_xsX( m, indbase,
						ptr, ind, val, alpha, xp, rowinc );
		}
		else /* ! is_sorted */
		{
			if( rowinc == 1 )
				for( k = 0, xp = x; k < num_vecs; k++, xp += colinc )
					CSR_MatConjTrisolveLower_Diag_Unsorted_xs1( m, indbase,
						ptr, ind, val, alpha, xp );
			else /* rowinc != 1 */
				for( k = 0, xp = x; k < num_vecs; k++, xp += colinc )
					CSR_MatConjTrisolveLower_Diag_Unsorted_xsX( m, indbase,
						ptr, ind, val, alpha, xp, rowinc );
		}
	}
}
