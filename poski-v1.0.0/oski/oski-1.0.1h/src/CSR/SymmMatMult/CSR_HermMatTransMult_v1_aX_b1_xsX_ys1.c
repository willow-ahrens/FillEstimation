 ...
 *  where \f$A\f$ is Hermitian (i.e., \f$A = A^H\f$), \f$op(A) = A^T\f$, \f$\alpha = \f$ (a general value),
 *  x is general-stride accessible, and y is unit-stride accessible.
 *  \ingroup MATTYPE_CSR
 */
 ...
 */
void
CSR_HermMatTransMult_v1_aX_b1_xsX_ys1( oski_index_t m, oski_index_t n,
	...
	oski_value_t* restrict y  )
{
	...
	for( i = 0, py = y, px = x; i < m; i++, px += incx, py++ )
	{
		oski_index_t k;
		...
		for( ; k < ptr[i+1]-index_base-1; k++ )
		{
			...
			/* y(i) += opc(A(i, j)) * x(j) */
			VAL_MAC_CONJ( _y0, a_ij, x[j * incx] );
				/* _y0 += a_ij * x[j * incx]; */

			/* y(j) += a_ij * alpha * x(i) */
			VAL_MAC( y[j], a_ij, _x0 );
				/* y[j] += opc(a_ij) * _x0; */
		}
...
/* finished on: Fri Feb 25 13:16:20 PST 2005 */
/* eof */
