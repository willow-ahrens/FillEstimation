/**
 *  \file src/heur/mregblock_eval.c
 ...
 *  instantiated type.
 */
static oski_profile_MBCSRMatMult_t g_profile = { 0, 0, NULL };
...
#define PROFILE_FILE "profile_MBCSR_MatMult_T" \
 ...
 *  This implementation 
 */
const oski_profile_MBCSRMatMult_t *
oski_LoadProfileMBCSRMatMult (const char *filename)
{
...
void
oski_UnloadProfileMBCSRMatMult (void)
{
 ...
 *  \ref oski_mregblock_t.
 *
 *  \returns Returns a \ref oski_mregblock_t structure if the
...
oski_HeurEvaluate (const oski_matrix_t A)
{
  ...
  results = oski_Malloc (oski_mregblock_t, 1);
  if (results == NULL)
    {
...
{
  const oski_mregblock_t *block_size = (const oski_mregblock_t *) results;
  char *xform;
  int err;
