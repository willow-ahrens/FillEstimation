/*
** $Id: lbaselib.c,v 1.1.1.1 2005/06/29 22:08:43 rvuduc Exp $
** Basic library
...
#pragma WARNING "Did not define a default Lua search path"
...
static int luaB_tryrequire (lua_State *L) {
  const char *path;
  int status = LUA_ERRFILE;  /* not found (yet) */
  luaL_checkstring(L, 1);
  lua_settop(L, 1);
