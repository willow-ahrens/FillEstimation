 ...
 *  where \f$A\f$ is symmetric (i.e., \f$A = A^T\f$), \f$op(A) = A\f$, \f$\alpha = -1\f$,
 *  x is unit-stride accessible, and y is general-stride accessible.
 *  \ingroup MATTYPE_CSR
 */
 ...
 */
void
CSR_SymmMatMult_v1_aN1_b1_xs1_ysX( oski_index_t m, oski_index_t n,
	...
	oski_value_t* restrict y , oski_index_t incy )
{
	...
	for( i = 0, py = y, px = x; i < m; i++, px++, py += incy )
	{
		oski_index_t k;
		...
		for( ; k < ptr[i+1]-index_base-1; k++ )
		{
			...
			/* y(i) += opc(A(i, j)) * x(j) */
			VAL_MAC( _y0, a_ij, x[j] );
				/* _y0 += a_ij * x[j]; */

			/* y(j) += a_ij * -x(i) */
			VAL_MAC( y[j * incy], a_ij, _x0 );
...
/* finished on: Fri Feb 25 13:16:17 PST 2005 */
/* eof */
