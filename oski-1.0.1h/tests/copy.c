/**
 *  \file tests/copy.c
 *  \brief Test matrix and vector handle copying.
 */
...
test_should_pass_1 (void)
{
  ...
  oski_matrix_t A_copy = oski_CopyMat (A_tunable);

  /* Create a common source vector */
  oski_vecview_t x0 = create_vector (x, 3);
  oski_vecview_t x1 = oski_CopyVecView (x0);
  ...
  if (A_copy == INVALID_MAT)
    {
      oski_PrintDebugMessage (1, "*** Matrix copy failed! ***");
      exit (1);
    }

  if (x1 == INVALID_VEC)
    {
      oski_PrintDebugMessage (1, "*** Vector view copy failed! ***");
      exit (1);
    }
  ...
  /* Method 2 */
  err = oski_MatTransMatMult (A_copy, OP_AT_A, alpha, x1, beta, y1, t1);
