...
for ax_openmp_flag in $ax_openmp_flags; do
  	...
  	AC_TRY_LINK_FUNC(omp_set_num_threads,[ax_cv_[]_AC_LANG_ABBREV[]_openmp=$ax_openmp_flag; break])
done
...
if test "x$ax_cv_[]_AC_LANG_ABBREV[]_openmp" = "xunknown"; then
	...
	if test "x$ax_cv_[]_AC_LANG_ABBREV[]_openmp" != "xnone"; then
  	...
  	m4_default([$1], [AC_DEFINE(HAVE_OPENMP_H,1,[Define if OpenMP is enabled])])
	ax_openmp_ok=yes 	
fi
