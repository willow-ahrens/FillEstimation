...
Operations on tensors, which are multidimensional analogues of matrices and vectors, are fundamental to many computational fields, ranging from machine learning to computational climate modeling.
Many tensors in application domains are sparse, and consist mostly of zero entries.
Substantial performance gains can be realized by taking advantage of the sparse structure, by avoiding unnecessary work.
However, the increased complexity of data structures for storing sparse tensors poses other performance challenges.
